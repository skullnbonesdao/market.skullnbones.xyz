...
//     return createToast("Wallet not connected", { type: "danger" });
//   if (globalStore.symbol.mint_asset === undefined)
//     return createToast("Asset not selected", { type: "danger" });
//   const availableTokenData = globalStore.userTokens.find(
//     (userToken) =>
//       userToken.mint === useGlobalStore().symbol.mint_pair.toString()
//   );
//   if (
//     input.value.size.text_box_value === 0 ||
//     input.value.price.text_box_value === 0
//   )
...
//     return createToast("No available token data on this wallet", {
//       type: "danger",
//     });
//   const availableTokens =
//     availableTokenData.data.account.data?.parsed?.info?.tokenAmount?.uiAmount;
//
//   if (availableTokens === undefined)
//     return createToast("Empty token data", { type: "danger" });
//
//   console.log(input.value.size.toString());
//
//   const neededTokens =
//     input.value.size.text_box_value * input.value.price.text_box_value;
...
//           useGlobalStore().symbol.name.includes(currency.name)
//         )?.name
//       }`,
//       { type: "danger" }
//     );
...
//       (asset) =>
//         asset.account.data?.parsed?.info?.mint ===
//         globalStore.symbol.mint_asset.toString()
//     );
//     const selectedUserAssetAmount =
//       selectedUserAsset?.account?.data?.parsed?.info?.tokenAmount?.uiAmount;
//     if (selectedUserAssetAmount < input.value.size)
//       return createToast(`Not enough ${globalStore.symbol.name}`, {
//         type: "danger",
//       });
//   }
//
//   const { transaction, signers } =
//     await staratlasGmStore.getInitializeOrderTransaction(
//       publicKey.value,
//       globalStore.symbol.mint_asset.toString(),
...
//     await solanaNetworkStore.connection.getLatestBlockhash();
//   solanaNetworkStore.connection
//     .confirmTransaction(
//       {
